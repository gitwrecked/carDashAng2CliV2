...
export class UsersService {
  ...
  addUserPurchase(email: string, purchase:Purchase ): Promise<Purchase> {    
    let options = new RequestOptions({ headers: headers });
    return this.http
       .post('/api/v1/user/', JSON.stringify({ email: email ,purchase:purchase}), options)
       .toPromise()
  ...
  create(name: string, amount:number,description:string,item:string ): Promise<Purchase> {    
    let options = new RequestOptions({ headers: headers });    
    return this.http
       .post('/api/v1/purchase/', JSON.stringify({ user: name ,amount:amount ,description:description, item: item}), options)
       .toPromise()
       .then(res => res.json().data)
       .catch(this.handleError);
  };
